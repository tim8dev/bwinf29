...
/**
 * Ein Robutton mit [fester] Geschwindigkeit und [veränderbare] Bewegungsrichtung.
 * @author Tim Taubner
 */
@ThreadSafe
...
public boolean collidesWith(Button o) {
    //  Prüfen ob \vec{V}=o.loc-this.loc o, etwa gleiche Richung wie \vec{dir} hat.
    Vec2 diff = o.getLoc().sub(getLoc());
    if(diff.angleBetween(dir.get()) < HALF_PI) // Wenn Richtungs abweichung nicht < 90°
...
public String toString() {
    ...
    return "Robutton{loc=" + getLoc() + ",dir=" + dir + 
            ",r=" + format.format(getRadius()) + ",v=" + format.format(mps) + '}';
}
