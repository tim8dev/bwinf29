...
public void addCoin(Coin c, Robutton r) {
    log.log(Level.INFO, "Coin dropped by {0}.", r);
    coins.add(c);
}

public void remCoin(Coin c, Robutton r) {
    log.log(Level.INFO, "Coin picked up by {0}.", r);
    coins.remove(c);
}
        ...
        public boolean update() {
            if (table.collidesWith(rob))
                if (log.isLoggable(Level.FINE))
                    log.log(Level.FINE, "{0} kollidierte mit {1} bei {2}", new Object[]{rob, this, rob.getLoc()});
            return false;
        }
        });
        ...
        public boolean update() {
            for (Robutton r_1 : robs)
                if (rob.collidesWith(r_1)) {
                    rob.onCollision(r_1);
                    if (log.isLoggable(Level.FINE))
                        log.log(Level.FINE, "{0} kollidierte mit {1} bei {2}", new Object[]{rob, r_1, rob.getLoc()});
                }
            return false;
        }
        });
        ...
        public boolean update() {
            for (Coin coin : coins)
                if (rob.collidesWith(coin)) {
                    ...
                    if (log.isLoggable(Level.FINE))
                        log.log(Level.INFO, "{0} kollidierte mit {1} bei {2}", new Object[]{rob, coin, rob.getLoc()});
                }
            return false;
        }
        });
