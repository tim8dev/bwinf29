...
class EinfachWuerfel(private val seiten : Int = 6) extends Wuerfel {
  ...
  def gesamtHaeufigkeit : Double = seiten
...
class DoppelWuerfel(private val wuerfel : Wuerfel) extends Wuerfel {
  private[this] lazy val moeglichkeiten = for(x <- wuerfel.zahlen; y <- wuerfel.zahlen)
    yield x+y
  lazy val zahlen = moeglichkeiten.toSet.toList
  
  def wuerfeln() = wuerfel.wuerfeln() + wuerfel.wuerfeln()
  def gesamtHaeufigkeit : Double = wuerfel.gesamtHaeufigkeit * wuerfel.gesamtHaeufigkeit
  def haeufigkeit(zahl: Int) = moeglichkeiten.filter(_==zahl).length
}
