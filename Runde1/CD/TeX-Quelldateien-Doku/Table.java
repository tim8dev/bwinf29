...
/**
 * Abstrakter Tisch mit Robuttons und Münzen(Coins)
 *
 * @author Tim Taubner
 */
...
private void createRobCollisionUpdates(final Robutton rob) {
    ...
    try {
        Updater.UPDATER.execute(new UpdateTask(800) {
        public boolean update() {
            table.collidesWith(rob)
            return false;
        }
        });
        Updater.UPDATER.execute(new UpdateTask(60) {
        public boolean update() {
            for (Robutton r_1 : robs)
                if (rob.collidesWith(r_1)) rob.onCollision(r_1);
            return false;
        }
        });
        Updater.UPDATER.execute(new UpdateTask(600) {
        public boolean update() {
            for (Coin coin : coins)
                if (rob.collidesWith(coin)) rob.onCollision(coin);
            return false;
        }
        });
