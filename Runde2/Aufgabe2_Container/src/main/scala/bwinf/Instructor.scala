...
object Instructor {
  ...
  def computeFromCycles(cycles: List[List[Int]]): Seq[Instruction] =
  ...
  private def computeCycle(cycle: List[Int], other: List[List[Int]]): (ListBuffer[Instruction], List[List[Int]]) = {
    val max = cycle.max // O(n_c)
    // Helper function:
    @tailrec def step(instrs: ListBuffer[Instruction], cyclesLeft: List[List[Int]],
                      prev: Int, cur: Int): (ListBuffer[Instruction], List[List[Int]], Int) =
      cyclesLeft.headOption match {
        // Does another Cycle begins directly behind the max of this cycle?
        case Some(nextCycle @ (next :: _)) if prev == max && next == max +1 => // (1)
          val (cycleInstrs, newCyclesLeft) = computeCycle(nextCycle, cyclesLeft.tail)
          val extraInstrs = instrs ++= // "Concat" Cycles
            ListBuffer(MoveRight, Rotate, PutWag, TakeCon) ++=
            cycleInstrs.init ++= // replace final PutWag
            ListBuffer(Rotate, TakeWag, Rotate, PutWag, MoveLeft)
          step(extraInstrs, newCyclesLeft, prev, cur)
        // Does another Cycle begins between prev and cur?
    ...
    //if(cycle.tail.nonEmpty) {
    val initial = (ListBuffer[Instruction](), other, first)
    val (instrs, cyclesLeft, last) = (initial /: cycle.tail) {
      case ((instrs, cyclesLeft, prev), cur) =>
        step(instrs, cyclesLeft, prev, cur)
    }
    // Then go back to first.
    step(instrs, cyclesLeft, last, first) match {
      case (instrs, cyclesLeft, _) => (instrs.init, cyclesLeft)
    }
    /*} else if(other.isEmpty) {
      ListBuffer[Instruction](Rotate, PutWag)
    } else {
      ListBuffer[Instruction](Rotate, PutWag, MoveRight, TakeCon) ++=
        computeCycle(other.head, other.tail)
    }*/
  }
   */
}
