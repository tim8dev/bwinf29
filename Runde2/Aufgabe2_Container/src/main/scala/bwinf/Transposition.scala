...
object Utils {
  ...
  def cyclesOf(perm: Seq[Int]) = cycles(perm) //filterNot (_.tail.isEmpty)

  private def cycles(perm: Seq[Int], handled: Set[Int] = Set(), start: Int = 1): Cycles =
    if(start > perm.length || start < 1) Nil
    else {
      val trans = cycle(perm, start)
      val newHandled = handled ++ trans
      val next = (start to perm.length) findIndexOf (i => !(newHandled contains i))
      if(next >= 0)
        trans :: cycles(perm, newHandled, next+start)
      else
        trans :: Nil
    }
  private def cycle(perm: Seq[Int], start: Int) = {
  ...
  private def helper(perm: Seq[Int], start: Int)(idx: Int): Cycle =
    if(start == idx) Nil
